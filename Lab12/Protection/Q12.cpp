...
void checkPermissions(const char *path)
{
    ...
    cout << "Read(R_OK) : " << (access(path, R_OK) == 0 ? "ALLOWED" : "DENIED") << endl;
    cout << "Write(W_OK) : " << (access(path, W_OK) == 0 ? "ALLOWED" : "DENIED") << endl;
    cout << "Execute(X_OK) : " << (access(path, X_OK) == 0 ? "ALLOWED" : "DENIED") << endl;
    cout << "Exists(F_OK) : " << (access(path, F_OK) == 0 ? "YES" : "NO") << endl;
}
